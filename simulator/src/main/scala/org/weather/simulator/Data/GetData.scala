...
object GetData {
  ...
  def getMappingData(): Map[String, ConstData.codes] = {
    val mapping = for (
      line <- ConstData.cities.map(x => x.split(ConstData.defaultFieldSep))
    ) yield (line(0), ConstData.codes(line(1), line(2)))
    mapping.toMap
  }
  ...
  def fileDownloader(date: Int, mapping: ConstData.codes) {
    val fileCode = mapping.fileCode
    val IATACode = mapping.areaCode
    val url = ConstData.siteUrl + date + "/text/" + fileCode + "." + date + ".csv"
    try {
      new URL(url) #> new File(fileCode) !!
    } catch {
